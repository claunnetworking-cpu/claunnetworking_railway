...
export default function AdminFilters() {
  ...
  // Gerar filtros dinâmicos por categoria de vagas
  const jobCategoryFilters = useMemo(() => {
    if (!jobsData) return [];
    const categories = new Set(jobsData.map((job: any) => job.category).filter(Boolean));
    return Array.from(categories).map(category => ({
      label: `Vagas - ${category.charAt(0).toUpperCase() + category.slice(1)}`,
      path: '/jobs',
  ...
  const jobFilters = [...jobFiltersBase, ...jobModalityFilters, ...jobStateFilters, ...jobCategoryFilters];
    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto p-8">
        <div className="flex items-center gap-4 mb-8">
          ...
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900">Filtros</h1>
        </div>
