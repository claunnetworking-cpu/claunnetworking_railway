...
export default function AdminLinks() {
  ...
  const getItemName = (link: any) => {
    if (link.resourceType === 'job') {
      const job = jobs.find(j => j.id === link.resourceId);
      return job?.title || 'Link personalizado';
    } else {
      const course = courses.find(c => c.id === link.resourceId);
      return course?.title || 'Link personalizado';
    }
  };

  const getItemType = (link: any) => {
    return link.resourceType === 'job' ? 'Vaga' : 'Curso';
  };
